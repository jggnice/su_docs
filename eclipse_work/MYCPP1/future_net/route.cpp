...
void search_routeA(int (*topo_int)[4], int sour, int vim)
{
	//// local variables ////
	int notValid[600] =
	{ 0 };
...
//vim no less than 3;no more than points
	search_routeA(topo_int, 0, 12);
	for (i = 2; i < num_of_demand; i++)
		search_routeA(topo_int, i, 12);
//////////////////   The Ending   ////////////////////
//////////////////////////////////////////////////////
	AA[1][0] = 0;
...
//		int tep = oldPopulation[k][num_of_demand - 1];
//		oldPopulation[k][num_of_demand - 1] = oldPopulation[k][1];
//		oldPopulation[k][1] = tep;
	}
	for (k = 0; k < SCALE; k++)
	{
		for (i = 0; i < num_of_demand; i++)
		{
			...
			int tep = oldPopulation[k][r];
			oldPopulation[k][r] = oldPopulation[k][i];
			oldPopulation[k][i] = tep;
//			int r = rand() % (num_of_demand - 1);
//			int tep = oldPopulation[k][r + 1];
//			oldPopulation[k][r + 1] = oldPopulation[k][1];
//			oldPopulation[k][1] = tep;
		}
	}
}
...
int evaluate()
{
	...
	for (i = 0; i < num_of_demand; i++)
	{
		printf("__%2d", oldPopulation[GOOD][i]);
	}
	printf("\n");
	return flag;
}
int select()
{
	memset(checkpoints, 0, sizeof(int) * 600);
	int i, j, k, n;
	n = 0;
	for (i = 0; i < num_of_demand; i++)
	{
